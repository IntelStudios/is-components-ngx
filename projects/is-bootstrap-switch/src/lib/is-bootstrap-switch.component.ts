...
const CUSTOM_INPUT: any = {
  ...
  useExisting: forwardRef(() => IsBootstrapSwitchComponent),
  multi: true
};

@Component({
  selector: 'is-bs-switch',
  ...
  template: `
      <div [ngClass]="getWrapperClasses()" [style.width]="(handleWidth  + labelWidth ) +'px'">
          <div #container [ngClass]="baseClass + '-container'"
              ...
              --><span #on [innerHTML]="(inverse ? offText : onText) | translate"
                [ngClass]="(inverse) ? getOffClasses() : getOnClasses()"></span><!--
              --><span #label [ngClass]="baseClass + '-label'">&nbsp;{{ labelText }}</span><!--
              --><span #off [innerHTML]="(inverse ? onText : offText) | translate"
...
export class IsBootstrapSwitchComponent implements AfterViewInit, ControlValueAccessor, OnChanges {
  ...
  // when inner state is null, then set to initValue
  @Input('initValue')
  initValue: boolean = false;

  // when set to true, then output values are "0" or "1"
  @Input('stringMode')
  ...
  constructor(@Optional() @Inject(configToken) private bsConfig: IsBootstrapSwitchConfig, private cd: ChangeDetectorRef, private render: Renderer2) {
    if (bsConfig) {
      Object.keys(bsConfig).forEach(k => {
        this[k] = bsConfig[k];
      });
    }
  }
  ...
  private setStateValue(v: boolean): void {
    if (v !== this._innerState) {
      ...
      this.stringMode ? this._onChangeCallback(String(Number(v))) : this._onChangeCallback(v);
