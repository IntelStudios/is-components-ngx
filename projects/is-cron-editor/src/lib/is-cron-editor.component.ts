...
export class IsCronEditorComponent implements OnInit, ControlValueAccessor, Validator {
  ...
  randomAbleSelectTypeValues: { ID: string, Value: string }[];
    ...
    try {
      ...
      if (seconds === '*') {
        ...
        if (seconds.indexOf('/') > -1 || seconds.indexOf('-') > -1) {
          ...
          this.formControl.seconds.specific.setValue(mapNumbers(seconds.split(',')).map((v) => `${v + 1}`));
        }
      }
      ...
      if (hours === '*') {
        ...
        if (hours.indexOf('/') > -1 || hours.indexOf('-') > -1) {
          ...
          this.formControl.hours.specific.setValue(mapNumbers(hours.split(',')).map((v) => `${v + 1}`));
        }
      }
